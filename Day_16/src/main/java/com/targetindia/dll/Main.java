    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 4: //Search
                    try {
                        int val = KeyboardUtil.getInt("Enter the element to be searched : ");
                        int j = list.search(val);
                        System.out.println("Element " + val + " found at index " + j);
                    } catch (Exception e) {
                ...
                case 7: //Insert data at a given index
                    try {
                        int idx = KeyboardUtil.getInt("Enter the index where data to be inserted : ");
                        int val = KeyboardUtil.getInt("Enter the data to be inserted : ");
                        list.insertAtIndex(idx, val);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
                    break;

                case 8: //Delete data at a given index
                    try {
                        int idx = KeyboardUtil.getInt("Enter the index where data to be deleted : ");
                        int deleteData = list.deleteAtIndex(idx);
                        System.out.println("Deleted Data : "+deleteData);
                    } catch (Exception e) {
